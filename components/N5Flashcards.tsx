    ...
    <div className="max-w-2xl mx-auto space-y-6 animate-fade-in pb-12">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-end gap-4">
        <div>
            ...
            <p className="text-slate-500 text-sm mt-1">Essential words for beginners</p>
        </div>
        ...
        <div className={`relative w-full h-full duration-500 transform-style-3d transition-all ease-out ${isFlipped ? 'rotate-y-180' : ''} ${feedback === 'correct' ? 'translate-x-[120%] opacity-0 rotate-12' : ''} ${feedback === 'incorrect' ? '-translate-x-[120%] opacity-0 -rotate-12' : ''}`}>
            ...
            >
                ...
                <div className="text-center space-y-8 z-10">
                    <span className="inline-block text-[10px] font-bold uppercase tracking-[0.2em] text-emerald-400 bg-emerald-50 px-3 py-1 rounded-full">
                        {mode === N5FlashcardMode.KANJI_TO_MEANING ? 'Kanji' : 'Definition'}
                    </span>
                    <h2 className={`font-bold text-slate-800 tracking-tight group-hover:scale-105 transition-transform duration-300 ${mode === N5FlashcardMode.KANJI_TO_MEANING ? 'text-6xl md:text-8xl font-jp' : 'text-4xl md:text-5xl font-mm'}`}>
                        {currentCard.front}
                    </h2>
                </div>
                
                <div className="absolute bottom-0 w-full p-6 flex justify-center bg-gradient-to-t from-white to-transparent pt-12">
                    <button className="text-slate-400 text-xs font-bold uppercase tracking-wider animate-pulse hover:text-emerald-500 transition-colors">
                        Click to Flip
                    </button>
                </div>
            </div>
            ...
            <div className="absolute inset-0 bg-slate-900 rounded-3xl shadow-2xl backface-hidden rotate-y-180 flex flex-col border border-slate-700 overflow-hidden">
                 <div className="absolute top-0 right-0 w-64 h-64 bg-emerald-600 rounded-full mix-blend-overlay filter blur-[80px] opacity-30 pointer-events-none"></div>
                 <div className="absolute bottom-0 left-0 w-64 h-64 bg-teal-600 rounded-full mix-blend-overlay filter blur-[80px] opacity-30 pointer-events-none"></div>
                 ...
                 <div className="relative z-10 flex-1 flex flex-col items-center justify-center p-8 text-center w-full">
                    <div className="w-12 h-12 bg-white/10 rounded-full flex items-center justify-center mb-6 text-emerald-300 backdrop-blur-sm shadow-inner">
                        <Sparkles size={20} />
                    </div>
                    <h2 className={`font-bold text-white leading-relaxed mb-6 drop-shadow-md ${mode === N5FlashcardMode.KANJI_TO_MEANING ? 'text-3xl md:text-4xl font-mm' : 'text-5xl md:text-7xl font-jp'}`}>
                        {currentCard.backMain}
                    </h2>
                    <p className="text-lg text-emerald-200 font-jp font-medium bg-white/10 px-5 py-2.5 rounded-xl border border-white/5 backdrop-blur-sm shadow-sm">
                        {currentCard.backSub}
                    </p>
                </div>
                 ...
                 <div className="relative z-20 p-8 pb-10 flex items-center justify-center gap-8 bg-gradient-to-t from-slate-900 via-slate-900/80 to-transparent">
                    ...
                    <button 
                        ...
                        title="Flip Back"
                    >
                        <div className="w-12 h-12 rounded-full bg-slate-800 border border-slate-600 text-slate-400 group-hover:bg-emerald-600 group-hover:border-emerald-500 group-hover:text-white flex items-center justify-center transition-all duration-200 shadow-lg hover:shadow-emerald-500/40 hover:scale-110 active:scale-95">
                            <RotateCw size={20} />
                        </div>
                        <span className="text-[10px] font-bold uppercase tracking-wider text-slate-500 group-hover:text-emerald-400 transition-colors">Flip</span>
                    </button>
